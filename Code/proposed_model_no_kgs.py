...
class GCN_RoBERTa_Combined_Model(nn.Module):
    def __init__(self, roberta_model_name='roberta-base', gcn_hidden_dim=64, num_classes=128, num_heads=8):
...
gcn_roberta_weights_path = 'Only_Robert_No_Kgs/10.pt'
...
class CombinedModelWithAttention(nn.Module):
    def __init__(self, gcn_roberta_model, cnn_lstm_model, lstm_model, combined_hidden_dim=192, num_classes=2, num_heads=8):
        ...
        self.fc_equal_em = nn.Linear(128, 192)
        self.fc_equal_concat = nn.Linear(192, 192)

    def forward(self, bert_tokens, attention_mask, audio_data, lstm_data):
        roberta_proj = self.gcn_roberta(bert_tokens, attention_mask)
        gcn_roberta_features = roberta_proj
...
for epoch in range(num_epochs):
    ...
    for graph_data, bert_tokens, attention_mask, audio_data, lstm_data, label, label_name in train_loader:
        ...
        logits = model(bert_tokens, attention_mask, audio_data, lstm_data)
